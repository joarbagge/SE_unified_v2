% Spectral Ewald 2P, Rotlet demo with accuracy check
...
ts = tic();
outr = SE2P_Rotlet_real_space(x, t, opt);
outf = SE2P_Rotlet_fourier_space(x, t, opt);
u = outr.pot + outf.pot;
time = toc(ts);
...
% Compute reference solution using direct summation
  ...
  ur = SE2P_Rotlet_direct_real_cut_mex(x, t, opt);
  % Fourier space
  opt.layers = ceil(max(opt.grid_res*box)/2); % M/2 layers
  uf = SE2P_Rotlet_direct_fourier_mex(x, t, opt);
  uf_k0 = SE2P_Rotlet_direct_fourier_k0_mex(x, t, opt);
  % Put together
  u = ur + uf + uf_k0;
end
