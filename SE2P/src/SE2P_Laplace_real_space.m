function out = SE2P_Laplace_real_space(x, f, opt)
%SE2P_Laplace_real_space  Compute real-space part of the
%2-periodic Ewald sum for the electrostatic potential.
%
%   out = SE2P_Laplace_real_space(x, f, opt)
%
...
if opt.return_pot && (strcmp(opt.eval_idx, 'default') || numel(opt.eval_idx) > 0)
  ts = tic;
  out.pot = SE2P_Laplace_real_cut_cell_mex(x, f, opt.rc, opt.xi, opt.box);
...
if opt.return_potgrad && (strcmp(opt.eval_idx, 'default') || numel(opt.eval_idx) > 0)
  ts = tic;
  out.potgrad = SE2P_Laplace_real_cut_cell_grad_mex(x, f, opt.rc, opt.xi, opt.box);
  if ~strcmp(opt.eval_idx, 'default')
...
if opt.return_pot && numel(opt.eval_ext_x) > 0
  ts = tic;
  opt.layers = 1;
  out.extpot = SE2P_Laplace_direct_real_cut_ext_mex(x, f, opt);
  out.walltime.extpot = toc(ts);
end
if opt.return_potgrad && numel(opt.eval_ext_x) > 0
  ts = tic;
  opt.layers = 1;
  out.extpotgrad = SE2P_Laplace_direct_real_cut_ext_grad_mex(x, f, opt);
