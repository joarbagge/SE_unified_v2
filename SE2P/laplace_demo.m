% Spectral Ewald 2P, Laplace potential demo with accuracy check
...
ts = tic();
outr = SE2P_Laplace_real_space(x, f, opt);
outf = SE2P_Laplace_fourier_space(x, f, opt);
...
% Compute reference solution using direct summation
  ...
  ur = SE2P_Laplace_direct_real_cut_mex(x, f, opt);
  % Fourier space
  opt.layers = ceil(max(opt.grid_res*box)/2); % M/2 layers
  uf = SE2P_Laplace_direct_fourier_mex(x, f, opt);
  uf_k0 = SE2P_Laplace_direct_fourier_k0_mex(x, f, opt);
  us = SE2P_Laplace_direct_self_mex(x, f, opt);
  % Put together
  u = ur + uf + uf_k0 + us;
end
