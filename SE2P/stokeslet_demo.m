% Spectral Ewald 2P, Stokeslet demo with accuracy check
...
opt = SE_Stokeslet_params(box, f, 'periodicity', 2, 'AbsTol', tol, 'rc', rc);
...
ts = tic();
outr = SE2P_Stokeslet_real_space(x, f, opt);
outf = SE2P_Stokeslet_fourier_space(x, f, opt);
...
% Compute reference solution using direct summation
  opt = SE_Stokeslet_params(box, f, 'periodicity', 2, 'AbsTol', tol, 'xi', xi);
  assert(opt.rc <= min(box), 'rc (%g) cannot be larger than min(box) (%g)', opt.rc, min(box));
  % Real space
  opt.layers = 1; % assuming rc <= min(box), this is enough for real-space part
  ur = SE2P_Stokeslet_direct_real_cut_mex(x, f, opt);
  % Fourier space
  opt.layers = ceil(max(opt.grid_res*box)/2); % M/2 layers
  uf = SE2P_Stokeslet_direct_fourier_mex(x, f, opt);
  uf_k0 = SE2P_Stokeslet_direct_fourier_k0_mex(x, f, opt);
  us = SE2P_Stokeslet_direct_self_mex(x, f, opt);
  % Put together
  u = ur + uf + uf_k0 + us;
end
