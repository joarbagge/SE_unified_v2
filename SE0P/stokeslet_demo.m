% Spectral Ewald 0P (free space), Stokeslet demo with accuracy check
...
opt = SE_Stokeslet_params(box, f, 'periodicity', 0, 'AbsTol', tol, 'rc', rc);
...
ts = tic();
outr = SE0P_Stokeslet_real_space(x, f, opt);
pre_kernel = SE0P_Stokeslet_precompute_kernel_fft(opt);
outf = SE0P_Stokeslet_fourier_space(x, f, opt, pre_kernel);
...
% Compute reference solution using direct summation
  u = SE0P_Stokeslet_direct_full_mex(x, f, struct());
end
