...
#define GR 4*PI*( 1 - cos(Rk) )/kk
#define GR_k0 2*PI*RR
#elif BIHARMONIC_KERNEL
// aB==0, bB==0
//#define GR 4*PI*( (2-Rk*Rk)*cos(Rk) + 2*Rk*sin(Rk) - 2 )/(kk*kk)
//#define GR_k0 PI*RR*RR
// aB==-R, bB==0
//#define GR 4*PI*( 2*cos(Rk) + Rk*sin(Rk) - 2 )/(kk*kk)
//#define GR_k0 -PI*RR*RR/3
// aB==-0.5*R, bB==-0.5/R -- the best
#define GR 4*PI*( -cos(Rk) + 3*sin(Rk)/Rk - 2 )/(kk*kk)
...
void k_vectors(const mwSize, const double, double *);
                 ...
                 const int nrhs, const mxArray *prhs[])
{
    ...
    const double R = mxGetPr(R_TRUNC)[0];
    const double RR = R*R;
    ...
    // Compute k vectors
    double* restrict k0 = mxMalloc(grid[0]*sizeof(double));
    double* restrict k1 = mxMalloc(grid[1]*sizeof(double));
    double* restrict k2 = mxMalloc(grid[2]*sizeof(double));
    k_vectors(grid[0], box[0], k0);
    k_vectors(grid[1], box[1], k1);
    k_vectors(grid[2], box[2], k2);

    // Compute modified Green's function
    double kk, k, Rk, grval;
...
void k_vectors(const mwSize M, const double L, double *k)
{
    double c = 2*PI/L;
    mwSize Mh = M/2;
