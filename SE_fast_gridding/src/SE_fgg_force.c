			   ...
			   double zf_2[P_MAX])
{
    ...
    double z_base0 = exp(c2*h*t0[0]);
    double z_base1 = exp(c2*h*t0[1]);
    double z_base2 = exp(c2*h*t0[2]);
    ...
    zf_0[0] = -c2*(t0[0]-p_from*h);
    zf_1[0] = -c2*(t0[1]-p_from*h);
    zf_2[0] = -c2*(t0[2]-p_from*h);
	...
	zf_0[i] = -c2*(t0[0]-(p_from+i)*h);
	zf_1[i] = -c2*(t0[1]-(p_from+i)*h);
	zf_2[i] = -c2*(t0[2]-(p_from+i)*h);
    }
...
// -----------------------------------------------------------------------------
static
int fgg_expansion_2p_force(const double x[3], const double q,
                           const SE_FGG_params* params,
                           double z2_0[P_MAX],
                           double z2_1[P_MAX],
                           double z2_2[P_MAX],
