function out = SE3P_Stresslet_real_space(x, q, n, opt)
%SE3P_Stresslet_real_space  Compute real-space part of the
%3-periodic Ewald sum for the stresslet potential.
%
%   out = SE3P_Stresslet_real_space(x, q, n, opt)
%
...
if strcmp(opt.eval_idx, 'default') || numel(opt.eval_idx) > 0
  ts = tic;
  out.pot = SE3P_Stresslet_real_cut_cell_mex(x, q, n, opt.rc, opt.xi, opt.box);
...
if numel(opt.eval_ext_x) > 0
  ts = tic;
  opt.layers = 1;
  out.extpot = SE3P_Stresslet_direct_real_cut_ext_mex(x, q, n, opt);
