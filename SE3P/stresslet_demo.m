% Spectral Ewald 3P, Stresslet demo with accuracy check
...
opt = SE_Stresslet_params(box, q, n, 'AbsTol', tol, 'rc', rc);
...
ts = tic();
outr = SE3P_Stresslet_real_space(x, q, n, opt);
outf = SE3P_Stresslet_fourier_space(x, q, n, opt);
u = outr.pot + outf.pot;
time = toc(ts);
...
% Compute reference solution using direct summation
  opt = SE_Stresslet_params(box, q, n, 'AbsTol', tol, 'xi', xi);
  assert(opt.rc <= min(box), 'rc (%g) cannot be larger than min(box) (%g)', opt.rc, min(box));
  % Real space
  opt.layers = 1; % assuming rc <= min(box), this is enough for real-space part
  ur = SE3P_Stresslet_direct_real_cut_mex(x, q, n, opt);
  % Fourier space
  opt.layers = ceil(max(opt.grid_res*box)/2); % M/2 layers
  uf = SE3P_Stresslet_direct_fourier_mex(x, q, n, opt);
  uf_k0 = SE3P_Stresslet_direct_fourier_k0_mex(x, q, n, opt);
  % Put together
  u = ur + uf + uf_k0;
end
