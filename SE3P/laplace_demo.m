% Spectral Ewald 3P, Laplace potential demo with accuracy check
...
% Compute reference solution using direct summation
  ...
  ur = SE3P_Laplace_direct_real_cut_mex(x, f, opt);
  % Fourier space
  opt.layers = ceil(max(opt.grid_res*box)/2); % M/2 layers
  uf = SE3P_Laplace_direct_fourier_mex(x, f, opt);
  us = SE3P_Laplace_direct_self_mex(x, f, opt);
  % Put together
  u = ur + uf + us;
end
