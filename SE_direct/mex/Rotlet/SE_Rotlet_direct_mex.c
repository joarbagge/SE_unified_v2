...
#define T   prhs[1] // Source strengths
...
#define EWALD_KERNEL SE3P_Rotlet_direct_real
#define EWALD_TAG "3P_REAL"
#elif TWO_PERIODIC
#define EWALD_KERNEL SE2P_Rotlet_direct_real
#define EWALD_TAG "2P_REAL"
#elif ONE_PERIODIC
#define EWALD_KERNEL SE1P_Rotlet_direct_real
#define EWALD_TAG "1P_REAL"
#elif ZERO_PERIODIC
#define EWALD_KERNEL SE0P_Rotlet_direct_real
...
#ifdef EWALD_FULL
#ifdef ONE_PERIODIC
#define EWALD_KERNEL SE1P_Rotlet_direct_full
#define EWALD_TAG "1P_FULL"
#elif ZERO_PERIODIC
#define EWALD_KERNEL SE0P_Rotlet_direct_full
...
#ifdef EWALD_FOURIER
#ifdef THREE_PERIODIC
#define EWALD_KERNEL SE3P_Rotlet_direct_fourier
#define EWALD_TAG "3P_FOURIER"
#elif TWO_PERIODIC
#define EWALD_KERNEL SE2P_Rotlet_direct_fourier
#define EWALD_TAG "2P_FOURIER"
#elif ONE_PERIODIC
#define EWALD_KERNEL SE1P_Rotlet_direct_fourier
...
#ifdef TWO_PERIODIC
#define EWALD_KERNEL SE2P_Rotlet_direct_fourier_k0
#define EWALD_TAG "2P_FOURIER_K0"
#elif ONE_PERIODIC
#define EWALD_KERNEL SE1P_Rotlet_direct_fourier_k0
                 ...
                 int nrhs, const mxArray *prhs[] )
{
    // Check input
    if (nrhs != 3)
        mexErrMsgIdAndTxt("SE:DirectMEXOpt", "Wrong number of input arguments (x, t, opt)");

    // Input dimensions, X and T should be N-by-3
    if (mxGetNumberOfElements(X) > 0 && mxGetN(X) != 3)
        mexErrMsgIdAndTxt("SE:DirectMEXOpt", "x must be N-by-3");
    const size_t N = mxGetM(X);
    if (mxGetM(T) != N || mxGetN(T) != mxGetN(X))
        mexErrMsgIdAndTxt("SE:DirectMEXOpt", "t must be N-by-3");
    ...
    {
#ifdef EXTERNAL
        __PRINTF("\n[DIRECT ROTLET EWALD (%s EXT)] MEX N=(%d,%d) ", EWALD_TAG, N, num_eval);
#elif CUTOFF
        __PRINTF("\n[DIRECT ROTLET EWALD (%s CUT)] MEX N=(%d,%d) ", EWALD_TAG, N, num_eval);
#else
        __PRINTF("\n[DIRECT ROTLET EWALD (%s)] MEX N=(%d,%d) ", EWALD_TAG, N, num_eval);
#endif
    ...
    // Call kernel
#ifdef EWALD_SELF
    EWALD_KERNEL(u, num_eval, t, N, &opt);
#else
    EWALD_KERNEL(u, num_eval, x, t, N, &opt);
#endif
