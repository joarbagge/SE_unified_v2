% Spectral Ewald 1P, Laplace gradient demo with accuracy check
...
% Compute reference solution using direct summation
  ...
  ur = SE1P_Laplace_direct_real_cut_grad_mex(x, f, opt);
  % Fourier space
  opt.layers = ceil(max(opt.grid_res*box)/2); % M/2 layers
  uf = SE1P_Laplace_direct_fourier_grad_mex(x, f, opt);
  uf_k0 = SE1P_Laplace_direct_fourier_k0_grad_mex(x, f, opt);
  % Put together
  u = ur + uf + uf_k0;
end
