function out = SE1P_Stresslet_real_space(x, q, n, opt)
%SE1P_Stresslet_real_space  Compute real-space part of the
%1-periodic Ewald sum for the stresslet potential.
%
%   out = SE1P_Stresslet_real_space(x, q, n, opt)
%
...
if strcmp(opt.eval_idx, 'default') || numel(opt.eval_idx) > 0
  ts = tic;
  out.pot = SE1P_Stresslet_real_cut_cell_mex(x, q, n, opt.rc, opt.xi, opt.box);
...
if numel(opt.eval_ext_x) > 0
  ts = tic;
  opt.layers = 1;
  out.extpot = SE1P_Stresslet_direct_real_cut_ext_mex(x, q, n, opt);
