function out = SE1P_base_fourier_space(x, f, opt)
%SE1P_base_fourier_space  Compute Fourier-space part of a
%1-periodic Ewald sum, using the Spectral Ewald method.
%
...
%       :param opt.grid_add2: extra number to add to the grid
%           size in the first free (y) direction, see below; if
%           set to the special value 'cover_remainder', the grid
...
%           size in the second free (z) direction, see further
%           opt.grid_add2 and below
...
%           zero mode (default: 1+R/min(Lext))
...
%           opt.EffAbsTol)
%
%   The grid sizes in the free directions (M2 and M3) will be set to
%       M2 = opt.grid_res*opt.box(2) + opt.window_P + opt.grid_add2
%       M3 = opt.grid_res*opt.box(3) + opt.window_P + opt.grid_add3
%   i.e. both opt.window_P and opt.grid_add2 (or opt.grid_add3) are
%   added to the values provided by opt.grid_res.
%
...
% TODO/FIXME: This code seems to produce a very large error when
% box(2) is different from box(3).
...
if strcmp(opt.window, 'expsemicirc')
  ...
  ts = tic;
  pre_fft = SE1P_window_fft_precomp(opt);
...
H0 = cell([dim_in, 1]);
for i=1:prod(dim_in)
  [H{i}, Hl{i}, H0{i}] = SE1P_fftnd(H{i}, opt);
end
...
ts = tic;
for i=1:dim_out
  H{i} = real(SE1P_ifftnd(H{i}, Hl{i}, H0{i}, opt)); % should be real to eps accuracy
end
...
% Compute stokeslet adjustment (due to differences between
% opt.stokeslet_k0_constant and the constants used by the
% modified stokeslet kernel).
if strcmp(opt.kernel, 'stokeslet')
  lB_out = exp(opt.stokeslet_k0_constant);
  lH_out = lB_out / exp(1);
  %lB_in = out.opt.greens_truncation_R;
  lB_in = out.opt.greens_truncation_R*exp(1/2);
  lH_in = out.opt.greens_truncation_R;
  f_sum = sum(f);
  if ~isfield(opt, 'vico_variant') || opt.vico_variant == 2
    adjustment = log(lB_in/lB_out)/opt.box(1) * [0 2 2] .* f_sum ...
               + log(lH_in/lH_out)/opt.box(1) * [4 0 0] .* f_sum;
  elseif opt.vico_variant == 1
    adjustment = log(lB_in/lB_out)/opt.box(1) * [4 2 2] .* f_sum;
  else
    adjustment = [0 0 0];
  end
  out.opt.stokeslet_adjustment = -adjustment;
end
...
if strcmp(opt.kernel, 'laplace')
  fun = @SE1P_Laplace_scaling;
elseif strcmp(opt.kernel, 'rotlet')
  fun = @SE1P_Rotlet_scaling;
elseif strcmp(opt.kernel, 'stokeslet')
  fun = @SE1P_Stokeslet_scaling;
elseif strcmp(opt.kernel, 'stresslet')
  fun = @SE1P_Stresslet_scaling;
else
...
if strcmp(opt.window, 'gaussian')
  ...
  W_fast_gath = @(F,opt,S) SE_fg_int_split_mex_1p(0,F,opt,...
                                S.zs,S.zx,S.zy,S.zz,S.idx);
  % FIXME: The first argument to SE_fg_int_split_mex_1p is unused.
  % It is not possible to specify the evaluation points, so we
  % have to pick out the correct evaluation points at the end.
  W_fast_gath_grad = @(F,opt,S) SE_fg_int_split_force_mex_1p(x,F,opt,...
                                S.zs,S.zx,S.zy,S.zz,...
                                S.zfx,S.zfy,S.zfz,S.idx);
  W_plain_grid = @SE_fg_grid_mex_1p;
  W_plain_gath = @SE_fg_int_mex_1p;
  W_plain_gath_grad = @SE_fg_int_force_mex_1p;
elseif strcmp(opt.window, 'expsemicirc') || strcmp(opt.window, 'kaiser_exact') ...
  ...
  W_fast_gath = @(F,opt,S) SE_fg_int_split_kaiser_mex_1p(0,F,opt,...
                                S.zx,S.zy,S.zz,S.idx);
  % FIXME: The first argument to SE_fg_int_split_kaiser_mex_1p is unused.
  % It is not possible to specify the evaluation points, so we
  % have to pick out the correct evaluation points at the end.
  W_fast_gath_grad = @(F,opt,S) SE_fg_int_split_kaiser_force_mex_1p(x,F,opt,...
                                S.zx,S.zy,S.zz,...
                                S.zfx,S.zfy,S.zfz,S.idx);
  W_plain_grid = @SE_fg_grid_kaiser_mex_1p;
  W_plain_gath = @SE_fg_int_kaiser_mex_1p;
  W_plain_gath_grad = @SE_fg_int_kaiser_force_mex_1p;
else
