function opt = SE1P_check_options(opt)
%SE1P_check_options  Check option struct for Spectral Ewald.
%
%   opt = SE1P_check_options(opt)
%
%   For documentation of options, see "help SE1P_base_fourier_space"
...
if strcmp(opt.kernel, 'stokeslet')
  opt = set_default_option(opt, 'stokeslet_k0_constant', 0);
end
...
% Default value for grid_add2, grid_add3
if strcmp(opt.window, 'gaussian')
  opt = set_default_option(opt, 'grid_add2', 0.5*opt.window_P);
  opt = set_default_option(opt, 'grid_add3', 0.5*opt.window_P);
else
  opt = set_default_option(opt, 'grid_add2', 1.4*opt.window_P);
...
if strcmp(opt.window, 'kaiser_poly') && opt.window_shape_factor ~= 2.5
  warning('SE1P:PolynomialShapeFactor', ...
...
[opt.grid, opt.grid_res] = SE_compute_grid_size(opt.box, opt.grid_res, opt.base_factor, 1);
if opt.grid_res / old_grid_res > 2
  warning('SE1P:ChangedGridRes', 'grid_res increased by more than a factor 2 (from %g to %g)', ...
          old_grid_res, opt.grid_res);
end
...
if ischar(opt.grid_add2) && strcmp(opt.grid_add2, 'cover_remainder')
  deltaM2 = cover_remainder_method(opt);
else % default method
  deltaM2 = opt.window_P + opt.grid_add2;
end
if ischar(opt.grid_add3) && strcmp(opt.grid_add3, 'cover_remainder')
  deltaM3 = cover_remainder_method(opt);
else % default method
  deltaM3 = opt.window_P + opt.grid_add3;
end
% TODO/FIXME: Maybe deltaM should be adjusted by multiplying with opt.grid_res/old_grid_res?
opt.grid2 = opt.grid(2) + deltaM2;
opt.grid3 = opt.grid(3) + deltaM3;
opt.grid2 = opt.base_factor * ceil(opt.grid2 / opt.base_factor); % round up
opt.grid3 = opt.base_factor * ceil(opt.grid3 / opt.base_factor); % round up
opt.box2 = opt.h * opt.grid2; % adjust box side length
opt.box3 = opt.h * opt.grid3; % adjust box side length
dbox2_half = (opt.box2 - opt.box(2))/2;
dbox3_half = (opt.box3 - opt.box(3))/2;
LextOverL_2 = opt.box2 / opt.box(2);
LextOverL_3 = opt.box3 / opt.box(3);
% Store values in standard vectors as well
opt.grid(2) = opt.grid2;
...
wbox = [0 opt.box(1); -dbox2_half opt.box2+dbox2_half; -dbox3_half opt.box3+dbox3_half];
opt.free_offset = wbox(2:3,1);
opt.greens_truncation_R = sqrt(opt.box2^2 + opt.box3^2);
def_upsampling_zero = 1 + opt.greens_truncation_R/min(opt.box2, opt.box3);
def_upsampling_zero = ceil(def_upsampling_zero*10)/10; % round up slightly
opt = set_default_option(opt, 'upsampling_zero', def_upsampling_zero);
...
LextOverL = min(LextOverL_2, LextOverL_3);
A = opt.A_fun(opt.source_quantity, opt.xi, opt.box(1));
...
opt.local_modes_size = ceil(max(opt.local_modes_size, 0)); % round up
...
grid_s0 = opt.base_factor * ceil([opt.grid2, opt.grid3]*s0 / opt.base_factor);
grid_sl = opt.base_factor * ceil([opt.grid2, opt.grid3]*sl / opt.base_factor);
grid_sg = opt.base_factor * ceil([opt.grid2, opt.grid3]*sg / opt.base_factor);
opt.actual_upsampling_zero = grid_s0 ./ [opt.grid2, opt.grid3];
opt.actual_upsampling_local = grid_sl ./ [opt.grid2, opt.grid3];
opt.actual_upsampling_global = grid_sg ./ [opt.grid2, opt.grid3];

% Defining the local modes to be upsampled ("local pad")
if ~all(opt.actual_upsampling_local == opt.actual_upsampling_global)
  ...
  opt.local_modes1 = [2:nx+1 Mx-nx+1:Mx];
else
  opt.local_modes1 = [];
end
...
% Method to determine deltaM from legacy SE0P code
  ...
  if strcmp(opt.window, 'gaussian')
    ...
    % TODO/FIXME: LEGACY Stokeslet 1P code seems to have had
    %deltaM_rem = sqrt(opt.window_shape/3)/opt.xi / opt.h;
  end
