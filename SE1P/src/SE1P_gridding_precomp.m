function pre_grid = SE1P_gridding_precomp(x, opt)
%SE1P_gridding_precomp  Precomputation of fast gridding data
%
%   pre_grid = SE1P_gridding_precomp(x, opt)
%
%   Input parameters:
...
if strcmp(opt.window, 'gaussian')
  ...
  pre_grid.zs = SE_fgg_base_gaussian_mex_1p(opt);
else
  if analytical_diff
    [zx, zy, zz, zfx, zfy, zfz, idx] = SE_fkg_expand_all_force_mex_1p(x, opt);
  else
