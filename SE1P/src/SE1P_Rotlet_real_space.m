function out = SE1P_Rotlet_real_space(x, t, opt)
%SE1P_Rotlet_real_space  Compute real-space part of the
%1-periodic Ewald sum for the rotlet potential.
%
%   out = SE1P_Rotlet_real_space(x, t, opt)
%
...
if strcmp(opt.eval_idx, 'default') || numel(opt.eval_idx) > 0
  ts = tic;
  out.pot = SE1P_Rotlet_real_cut_cell_mex(x, t, opt.rc, opt.xi, opt.box);
...
if numel(opt.eval_ext_x) > 0
  ts = tic;
  opt.layers = 1;
  out.extpot = SE1P_Rotlet_direct_real_cut_ext_mex(x, t, opt);
