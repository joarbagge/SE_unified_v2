function opt = SE_Rotlet_params(varargin)
%SE_Rotlet_params  Automatic parameter selection for the Rotlet solver
%
%   opt = SE_Rotlet_params(box, f, ...)
%       Creates a parameter selection struct `opt'.
%
%   opt = SE_Rotlet_params(opt, ...)
%       Extends an existing `opt' struct.
%
%   Required input parameters (first syntax):
%       :param box: size of periodic cell [L1,L2,L3]
%       :param f: source torques (N×3)
%
...
co = 11.60; CC = 2.42;
xiLfun = @(t) sqrt(t) .* exp(-co./t.^2);
opt.A_fun = @(Q, xi, L) CC * sqrt(Q) .* xiLfun(xi.*L) ./ L.^2;
...
if comp_rc_xi_grid_res
  Ffactor = 4*source_quantity./(3*V*EffAbsTol.^2);
end
...
if comp_rc_xi_grid_res && ~xi_given
  ...
  if grid_res_given
    xi_grid_res = xi_from_grid_res(opt.grid_res, Ffactor);
    opt.xi = xi_grid_res;
  end
...
if comp_rc_xi_grid_res && ~grid_res_given
  opt.grid_res = grid_res_from_xi(opt.xi, Ffactor);
...
if ~fix_applied && has_grid_res && has_xi && has_P && ~strcmp(opt.fix_policy, 'never')
  ...
  elseif strcmp(opt.fix_policy, 'indicator')
    ...
    tmp = -pi^2*X.^2/4 + 0.5*log(16*Q./(3*pi^2*L.^2));
    if strcmp(opt.window, 'gaussian')
      coeff = [-0.71, 2.1];
    else
      coeff = [-0.60, 1.5];
...
function grid_res = grid_res_from_xi(xi, factor)
  c = 2*xi.*factor/pi;
  grid_res = xi/pi .* sqrt(lambertw(c.^2));

function xi = xi_from_grid_res(grid_res, factor)
  c = factor.*grid_res;
  xi = pi*grid_res/sqrt(2) ./ sqrt(lambertw(c));
